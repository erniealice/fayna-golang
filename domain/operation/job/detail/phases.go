...
// Uses the job_phase.DetailURL constant directly (the job_phase module owns this route).
func phaseDetailURL(id string) string {
	return route.ResolveURL(job_phase.DetailURL, "id", id)
}
...
func phaseStatusLabel(status string, l job.Labels) string {
	switch status {
	case "pending":
