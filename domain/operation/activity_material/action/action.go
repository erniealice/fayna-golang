...
func buildFormData(record *activitymaterialpb.ActivityMaterial, routes activity_material.Routes, labels activity_material.Labels) *activitymaterialform.Data {
	productName := ""
...
func buildEmptyFormData(activityID string, routes activity_material.Routes, labels activity_material.Labels) *activitymaterialform.Data {
	return &activitymaterialform.Data{
		IsEdit:            false,
...
func addFormAction(routes activity_material.Routes) string {
	return routes.AddURL
}

// editFormAction returns the URL for the Edit form's hx-post attribute for a given activity ID.
func editFormAction(routes activity_material.Routes, activityID string) string {
	if activityID == "" {
