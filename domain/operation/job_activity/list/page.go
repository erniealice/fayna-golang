...
func activityColumns(l job_activity.Labels) []types.TableColumn {
...
func buildTableRows(activities []*jobactivitypb.JobActivity, l job_activity.Labels, routes job_activity.Routes, inUseIDs map[string]bool, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, a := range activities {
		id := a.GetId()
