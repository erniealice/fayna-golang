...
type Deps struct {
	Routes               job.Routes
	JobTemplateRoutes    job_template.Routes
...
func buildRecentActivityList(items []*jobactivitypb.JobActivity, l job.DashboardLabels) []types.ActivityItem {
	if len(items) == 0 {
		return nil
	}
...
func buildUpcomingDeadlinesHTML(jobs []*jobpb.Job, l job.DashboardLabels, routes job.Routes) template.HTML {
	if len(jobs) == 0 {
		return ""
	}
