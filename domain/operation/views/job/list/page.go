...
func jobColumns(l operation.JobLabels) []types.TableColumn {
...
func buildTableRows(jobs []*jobpb.Job, status string, l operation.JobLabels, routes operation.JobRoutes, inUseIDs map[string]bool, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, j := range jobs {
...
func statusPageTitle(l operation.JobLabels, status string) string {
	switch status {
	case "draft":
...
func statusPageCaption(l operation.JobLabels, status string) string {
	switch status {
	case "draft":
