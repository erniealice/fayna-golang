// Package job_task provides the view module for the JobTask entity.
//
// Shape 3 (sibling, per-handler split). Reached via JobPhase detail Tasks tab
// deep links (/app/job-task/{id}). The list page (/app/job-tasks/list/{status})
// is a power-user/debugging surface with no sidebar entry.
//
// JobPhase's Tasks tab is a thin proxy — it calls ListByPhase and renders
// each task row as a deep-link into this module.
package job_task
...
import (
	...
	jobtaskaction "github.com/erniealice/fayna-golang/domain/operation/views/job_task/action"
	jobtaskdetail "github.com/erniealice/fayna-golang/domain/operation/views/job_task/detail"
	jobtasklist "github.com/erniealice/fayna-golang/domain/operation/views/job_task/list"
)

// ModuleDeps holds all dependencies for the job_task module.
type ModuleDeps struct {
	Routes       operation.JobTaskRoutes
...
// Module holds all constructed job_task views.
type Module struct {
	routes           operation.JobTaskRoutes
	List             view.View
	Detail           view.View
...
// NewModule creates a new job_task module with all views wired.
