// placement_test.go — Wave-T placement gate for fayna (DOMAIN variant).
//
...
//
// fayna root retains assets.go (embed), labels.go, routes.go, routes_config.go
// as empty stubs (Wave-T stripped, not yet removed). views/ is an empty dir
// left over from the move. docs/ holds planning markdown. All excused in
// legacyAllow until a cleanup wave removes the stubs and empty dirs.
...
var crossCutting = false // fayna is a domain package
var legacyAllow = map[string]string{
	"assets.go":        "embed FS — legitimately at root; remove only when pyeza absorbs the asset hosting",
	"labels.go":        "empty stub post-Wave-T strip; remove when service-admin import is cleaned up",
	"routes.go":        "empty stub post-Wave-T strip; remove when service-admin import is cleaned up",
	"routes_config.go": "empty stub post-Wave-T strip; remove when service-admin import is cleaned up",
	"views":            "empty dir left by move; remove with 'rmdir views' after confirming no consumer",
	"docs":             "planning markdown; not a Go concern — excused from R2",
}
