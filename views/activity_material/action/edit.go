	...
	return view.ViewFunc(func(ctx context.Context, viewCtx *view.ViewContext) view.ViewResult {
		...
		if viewCtx.Request.Method == http.MethodGet {
			...
			if err != nil {
				...
				return fayna.HTMXError(fmt.Sprintf("Failed to load material record: %v", err))
			}
			data := resp.GetData()
			if len(data) == 0 {
		...
		if deps.UpdateActivityMaterial == nil {
			...
			return fayna.HTMXError("UpdateActivityMaterial use case not wired — add ActivityMaterial to espyna OperationUseCases")
		}
