...
import (
	...
	activityexpenseform "github.com/erniealice/fayna-golang/views/activity_expense/form"
...
type Deps struct {
	Routes fayna.ActivityExpenseRoutes
...
func buildPaymentMethodLabels(labels fayna.ActivityExpenseLabels) activityexpenseform.PaymentMethodLabels {
...
func buildFormData(record *activityexpensepb.ActivityExpense, routes fayna.ActivityExpenseRoutes, labels fayna.ActivityExpenseLabels) *activityexpenseform.Data {
...
func buildEmptyFormData(activityID string, routes fayna.ActivityExpenseRoutes, labels fayna.ActivityExpenseLabels) *activityexpenseform.Data {
	pmLabels := buildPaymentMethodLabels(labels)
	return &activityexpenseform.Data{
		IsEdit:                   false,
...
func addFormAction(routes fayna.ActivityExpenseRoutes) string {
	return routes.AddURL
}

// editFormAction returns the URL for the Edit form's hx-post attribute for a given activity ID.
func editFormAction(routes fayna.ActivityExpenseRoutes, activityID string) string {
	if activityID == "" {
