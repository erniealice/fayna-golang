...
func buildRecentActivityList(items []*jobactivitypb.JobActivity, l operation.JobDashboardLabels) []types.ActivityItem {
	if len(items) == 0 {
		return nil
	}
...
func buildUpcomingDeadlinesHTML(jobs []*jobpb.Job, l operation.JobDashboardLabels, routes operation.JobRoutes) template.HTML {
	if len(jobs) == 0 {
		return ""
	}
