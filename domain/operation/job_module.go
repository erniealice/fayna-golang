...
import (
	...
	jobdetail "github.com/erniealice/fayna-golang/domain/operation/job/detail"
	joblist "github.com/erniealice/fayna-golang/domain/operation/job/list"
)

// JobModuleDeps holds all dependencies for the job module.
type JobModuleDeps struct {
	Routes       jobpkg.Routes
	Labels       jobpkg.Labels
	...
	// Set from operation.JobRoutes by the fayna block (JobClientSearchURL / JobLocationSearchURL).
...
// JobModule holds all constructed job views.
...
// NewJobModule creates a new job module with all views wired.
func NewJobModule(deps *JobModuleDeps) *JobModule {
...
func (m *JobModule) RegisterRoutes(r view.RouteRegistrar) {
	// Phase 3 — Pyeza dashboard block + per-app live dashboards plan.
