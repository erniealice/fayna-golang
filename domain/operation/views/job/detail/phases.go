...
// Uses the operation.JobPhaseDetailURL constant directly (the job_phase module owns this route).
func phaseDetailURL(id string) string {
	return route.ResolveURL(operation.JobPhaseDetailURL, "id", id)
}
...
func phaseStatusLabel(status string, l operation.JobLabels) string {
	switch status {
	case "pending":
...
func buildPhasesTable(
	...
	columns := []types.TableColumn{
		{Key: "phase", Label: l.Detail.PhaseName},
		{Key: "order", Label: "#", NoSort: true, WidthClass: "col-sm"},
		{Key: "name", Label: l.Detail.TaskName},
		{Key: "status", Label: l.Detail.PhaseStatus, WidthClass: "col-2xl"},
		{Key: "assigned_to", Label: "Assigned To", NoSort: true, WidthClass: "col-2xl"},
	}
